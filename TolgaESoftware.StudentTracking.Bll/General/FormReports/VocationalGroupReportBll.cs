...
{
    public class VocationalGroupReportBll : BaseBll<Accrue, StudentTrackingContext>
    {
        public IEnumerable<VocationalGroupReportList> List(Expression<Func<Accrue, bool>> filter)
        {
            return BaseList(filter, p => new
            {
                Accrue = p,
                p.Branch,
                VocationId = p.ContactInformations.Where(c => c.Guardianship).Select(c => c.Contact.VocationId).FirstOrDefault(),
                VocationName = p.ContactInformations.Where(c => c.Guardianship).Select(c => c.Contact.Vocation.VocationName).FirstOrDefault(),
            ...
            }).AsEnumerable().GroupBy(p => new { p.VocationId, p.Accrue.BranchId }).Select(p => new VocationalGroupReportList
            {
                BranchName = p.Select(c => c.Accrue.Branch.BranchName).FirstOrDefault(),
                VocationName = p.Select(c => c.VocationName).FirstOrDefault() ?? "Meslek Girilmedi",
