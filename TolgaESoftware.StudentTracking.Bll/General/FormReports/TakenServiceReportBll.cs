...
{
    public class TakenServiceReportBll : BaseBll<Accrue, StudentTrackingContext>
    {
        public IEnumerable<TakenServiceReportList> List(Expression<Func<Accrue, bool>> filter,IEnumerable<long> serviceTypes,TakenServiceStatus takenServiceStatus)
        {
            return BaseList(filter, p => new
            {
                ...
                TakenServiceStatus = p.ServiceInformations.Where(c=>serviceTypes.Contains(c.ServiceId)&&!c.Cancelled).GroupBy(c=>c.AccrueId).Any()

            }).Where(p=>takenServiceStatus==TakenServiceStatus.TakenService?p.TakenServiceStatus:!p.TakenServiceStatus).Select(p => new TakenServiceReportList
            {
